...
exports.extractAgents = function (xmlDoc) {
  ...
  for (var aChild in children) {
    ...
    if (n.name() === 'name') {
      ...
      for (var aAttr in attrs) {
        var a = attrs[aAttr]
        if (a.name() === 'type') agent.type = a.value()
        if (a.name() === 'authority') agent.authority = a.value()
        if (a.name() === 'valueURI') agent.valueURI = a.value()
        if (a.name() === 'usage') agent.usage = a.value()
      }
        ...
        if (nn.name() === 'role') {
          for (var aGreatGrandChild in nn.childNodes()) {
            ...
            if (nnn.name() === 'roleTerm') {
              var attrs2 = nnn.attrs()

              for (var aAttr2 in attrs2) {
                var a2 = attrs2[aAttr2]
                if (a2.name() === 'valueURI') {
                  if (agent.role.indexOf(a2.value()) === -1) agent.role.push(a2.value())
                }
              }
            }
          }
  ...
  for (var aChild in children) {
    ...
    if (n.name() === 'subject') {
      for (var aGrandChild in n.childNodes()) {
        ...
        for (var aAttr in attrs) {
          var a = attrs[aAttr]
          if (a.name() === 'type') subject.nameType = a.value()
          if (a.name() === 'authority') subject.authority = a.value()
          if (a.name() === 'valueURI') subject.valueURI = a.value()
          if (a.name() === 'usage') subject.usage = a.value()
        }
...
exports.extractDivision = function (xmlDoc) {
  ...
  if (divisions.length === 0) {
    for (var div in divisionsLongForm) {
      for (var x in lexicon.labels.mmsDivisions) {
        if (lexicon.labels.mmsDivisions[x].name === divisionsLongForm[div]) {
          if (divisions.indexOf(x.toUpperCase()) === -1) divisions.push(x.toUpperCase())
        }
      }
    }
  }
...
exports.extractNotes = function (xmlDoc) {
  ...
  for (var aChild in children) {
    ...
    if (n.name() === 'note') {
      ...
      for (var aAttr in attrs) {
        var a = attrs[aAttr]
        if (a.name() === 'type') note.type = a.value()
      }
...
exports.extractTitles = function (xmlDoc) {
  ...
  for (var aChild in children) {
    ...
    if (n.name() === 'titleInfo') {
      ...
      for (var aAttr in attrs) {
        var a = attrs[aAttr]
        if (a.name() === 'type') title.type = a.value()
        if (a.name() === 'authority') title.authority = a.value()
        if (a.name() === 'usage') if (a.value() === 'primary') title.primary = true
        if (a.name() === 'lang') title.lang = a.value()
        if (a.name() === 'supplied') title.supplied = a.value()
        if (a.name() === 'script') title.script = a.value()
      }
    ...
    if (n.name() === 'originInfo') {
      for (var aGrandChild in n.childNodes()) {
        ...
        if (nn.name() === 'dateIssued' || nn.name() === 'dateCreated' || nn.name() === 'copyrightDate') {
          ...
          for (var aAttr in attrs) {
            var a = attrs[aAttr]
            if (a.name() === 'encoding') date.type = a.value()
            if (a.name() === 'keyDate') date.keyDate = a.value()
            if (a.name() === 'point') date.point = a.value()
          }
...
exports.extractGenres = function (xmlDoc) {
  ...
  for (var aChild in children) {
    ...
    if (n.name() === 'genre') {
      ...
      for (var aAttr in attrs) {
        var a = attrs[aAttr]
        if (a.name() === 'authority') genre.authority = a.value()
        if (a.name() === 'valueURI') genre.valueURI = a.value()
      }
  ...
  for (var aChild in children) {
    ...
    if (n.name() === 'physicalDescription') {
      ...
      for (var aGrandChild in n.childNodes()) {
        var nn = n.childNodes()[aGrandChild]
        if (nn.name() === 'form') {
          if (physicalDescription.form.indexOf(nn.text().trim()) === -1) physicalDescription.form.push(nn.text().trim())
        }
        if (nn.name() === 'extent') {
          if (physicalDescription.extent.indexOf(nn.text().trim()) === -1) physicalDescription.extent.push(nn.text().trim())
        }
      }
    ...
    if (n.name() === 'originInfo') {
      ...
      for (var aGrandChild in n.childNodes()) {
        ...
        if (nn.name() === 'publisher') {
          if (originInfo.publisher.indexOf(nn.text().trim()) === -1) originInfo.publisher.push(nn.text().trim())
        }
        ...
        if (nn.name() === 'place') {
          for (var aGreatGrandChild in nn.childNodes()) {
            ...
            if (nnn.name() === 'placeTerm') {
              if (originInfo.place.indexOf(nnn.text().trim()) === -1) originInfo.place.push(nnn.text().trim())
            }
          }
        }
      }
        ...
        if (mmsHash.mms_hash.mhash) {
          if (mmsHash.mms_hash.mhash.copyright) {
            ...
            if (mmsHash.mms_hash.mhash.copyright.holders) {
              for (var cc in mmsHash.mms_hash.mhash.copyright.holders) {
                cc = mmsHash.mms_hash.mhash.copyright.holders[cc]
                if (cc.holder_name || cc.holder_contact || cc.holder_note) {
                  rightsAgents.holders.push({
                    namePart: (cc.holder_name) ? cc.holder_name : false,
                    contact: (cc.holder_contact) ? cc.holder_contact : false,
                    note: (cc.holder_note) ? cc.holder_note : false
                  })
                }
              }
            }
          ...
          if (mmsHash.mms_hash.mhash.provenance) {
            if (mmsHash.mms_hash.mhash.provenance.donor) {
              if (mmsHash.mms_hash.mhash.provenance.donor !== '') rightsAgents.donor = mmsHash.mms_hash.mhash.provenance.donor
            }

            if (mmsHash.mms_hash.mhash.provenance.donor_contactinfo) {
              if (mmsHash.mms_hash.mhash.provenance.donor_contactinfo !== '') rightsAgents.donorContactInfo = mmsHash.mms_hash.mhash.provenance.donor_contactinfo
            }

            if (mmsHash.mms_hash.mhash.provenance.donor_relationship) {
              if (mmsHash.mms_hash.mhash.provenance.donor_relationship !== '') rightsAgents.donorRelationship = mmsHash.mms_hash.mhash.provenance.donor_relationship
            }
          }
        }
      }
